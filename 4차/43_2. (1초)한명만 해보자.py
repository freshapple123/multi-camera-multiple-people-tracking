...
while True:
    ...
    for cam_name in frames.keys():
        ...
        for person_idx in range(len(kp_array)):
            ...
            if frame_count % match_interval == 0:
                # -------- 실제 유사도 계산 --------
                ...
                if matched_id is not None:
                    person_db[matched_id]["last_matched_frame"] = frame_count
                    person_db[matched_id]["last_matched_id"] = matched_id
            else:
                # -------- 이전 매칭 유지 --------
                for pid, data in person_db.items():
                    if "last_position" in data and data["last_position"] and pid == 0:
                        if (
                            "last_matched_frame" in data
                            and frame_count - data["last_matched_frame"]
                            < match_interval
                        ):
                            matched_id = data["last_matched_id"]
                            center_pos = data["last_position"]
